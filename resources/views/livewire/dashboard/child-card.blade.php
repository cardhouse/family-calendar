...
return new class extends Component
{
    ...
    public bool $showCompletedCardDetails = false;

    public function mount(Child $child, array $activeAssignmentIds = [], ?bool $showCompleted = null): void
    {
        $this->child = $child;
        $this->activeAssignmentIds = $this->normalizeAssignmentIds($activeAssignmentIds);
        if (is_bool($showCompleted)) {
    ...
    public function toggleCompletion(int $assignmentId): void
    {
        ...
        if (! $this->isComplete) {
            $this->showCompletedCardDetails = false;
        }
    }

    public function expandCompletedCard(): void
    {
        if (! $this->isComplete) {
            return;
        }

        $this->showCompletedCardDetails = true;
    }

    public function collapseCompletedCard(): void
    {
        $this->showCompletedCardDetails = false;
    }
    ...
    {
        if ($this->isComplete && $this->showCompletedCardDetails) {
    ...
    #[Computed]
    public function shouldShowCompletedSummaryCard(): bool
    {
        return $this->isComplete && ! $this->showCompletedCardDetails;
    }
...
<div>
@if ($this->shouldShowCompletedSummaryCard)
    <div
        @class([
            'rounded-3xl border border-emerald-500/40 bg-emerald-500/10 p-5 shadow-sm',
            'animate-complete-glow' => $this->isComplete,
        ])
        style="border-top: 4px solid {{ $child->avatar_color }};"
    >
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div
                    class="flex h-12 w-12 items-center justify-center rounded-full text-xl font-black text-white shadow-lg"
                    style="background-color: {{ $child->avatar_color }};"
                >
                    {{ strtoupper(mb_substr($child->name, 0, 1)) }}
                </div>
                <div>
                    <flux:heading size="base" class="font-extrabold text-emerald-100">{{ $child->name }}</flux:heading>
                    <flux:text class="text-xs text-emerald-200/90">Completed for today</flux:text>
                </div>
            </div>
            <div class="rounded-full border border-emerald-400/40 bg-emerald-400/10 px-3 py-1 text-xs font-black tracking-wide text-emerald-100">
                100%
            </div>
        </div>

        <div class="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3">
            <div class="flex items-center gap-2 text-sm font-bold text-emerald-100">
                <flux:icon name="star" variant="solid" class="size-4 text-amber-300" />
                <span>Completed for today</span>
            </div>
            <flux:button wire:click="expandCompletedCard" variant="ghost" size="sm" class="font-bold text-emerald-100">
                View tasks
            </flux:button>
        </div>
    </div>
@else
    <div
        @class([
            'rounded-3xl border border-dash-border bg-dash-card p-6 shadow-sm transition-shadow',
            'animate-complete-glow' => $this->isComplete,
        ])
    ...
    >
        ...
        @if ($this->isComplete)
            <div class="mt-5 flex flex-wrap items-center justify-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-5 py-4 animate-celebration">
                ...
                <flux:icon name="star" variant="solid" class="size-6 text-amber-400" />
                <flux:button wire:click="collapseCompletedCard" variant="ghost" size="sm" class="font-bold">
                    Collapse card
                </flux:button>
            </div>
        @endif
